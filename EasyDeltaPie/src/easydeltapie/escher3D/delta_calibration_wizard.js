...
// This is the code i stole *ahem* replicated under the GPL license.
// You my boy Echer3D, you my boy..
...
function setPoints() {
	var rt = document.getElementById("inputTable");
	var nrows = document.getElementById("numPoints").value;
	while(rt.rows.length - numEarlierRows - numLaterRows > nrows) {
		rt.deleteRow(rt.rows.length - numLaterRows - 1);
	}
	while(rt.rows.length - numEarlierRows - numLaterRows < nrows) {
		var n = rt.rows.length - numLaterRows;
		var newRow = rt.insertRow(n);
		var cell = newRow.insertCell(0);
		var index = n - numEarlierRows;
		cell.innerHTML = "Point " + index;
		cell = newRow.insertCell(1);
		cell.innerHTML = "X:";
		cell = newRow.insertCell(2);
		cell.innerHTML = "<input type='number' value='0.0' step='0.01' style='width:70px' id='probeX" + index + "'>";
		cell = newRow.insertCell(3);
		cell.innerHTML = "Y:";
		cell = newRow.insertCell(4);
		cell.innerHTML = "<input type='number' value='0.0' step='0.01' style='width:70px' id='probeY" + index + "'>";
		cell = newRow.insertCell(5);
		cell = newRow.insertCell(6);
		cell.innerHTML = "Nozzle height error:";
		cell = newRow.insertCell(7);
		cell.innerHTML = "<input type='number' value='0.0' step='0.01' style='width:70px' id='probeZ" + index + "'>";
	}
	document.getElementById("suggestButton").disabled = (nrows != 4 && nrows != 7 && nrows != 10);
}
...
function calcProbePoints() {
	...
	if (numPoints == 4) {
		for (var i = 0; i < 3; ++i) {
			document.getElementById("probeX" + i).value = (bedRadius * Math.sin((2 * Math.PI * i)/3)).toFixed(2);
			document.getElementById("probeY" + i).value = (bedRadius * Math.cos((2 * Math.PI * i)/3)).toFixed(2);
		}
		document.getElementById("probeX" + 3).value = 0.0;
		document.getElementById("probeY" + 3).value = 0.0;
	}
	else {
		if (numPoints >= 7) {
			for (var i = 0; i < 6; ++i) {
				document.getElementById("probeX" + i).value = (bedRadius * Math.sin((2 * Math.PI * i)/6)).toFixed(2);
				document.getElementById("probeY" + i).value = (bedRadius * Math.cos((2 * Math.PI * i)/6)).toFixed(2);
			}
		}
		if (numPoints >= 10) {
			for (var i = 6; i < 9; ++i) {
				document.getElementById("probeX" + i).value = (bedRadius/2 * Math.sin((2 * Math.PI * (i - 6))/3)).toFixed(2);
				document.getElementById("probeY" + i).value = (bedRadius/2 * Math.cos((2 * Math.PI * (i - 6))/3)).toFixed(2);
			}
			document.getElementById("probeX" + 9).value = 0.0;
			document.getElementById("probeY" + 9).value = 0.0;
		}
		else {
			document.getElementById("probeX" + 6).value = 0.0;
			document.getElementById("probeY" + 6).value = 0.0;
		}
	}	
}
...
function setNewParameters() {
	...
	document.getElementById("newrodlength").value = deltaParams.diagonal.toFixed(2);
	document.getElementById("newradius").value = deltaParams.radius.toFixed(2);
	document.getElementById("newhomedheight").value = deltaParams.homedHeight.toFixed(2);
	document.getElementById("newxpos").value = deltaParams.xadj.toFixed(2);
	document.getElementById("newypos").value = deltaParams.yadj.toFixed(2);
	document.getElementById("newzpos").value = deltaParams.zadj.toFixed(2);
}
...
function copyToInitial() {
		document.getElementById("oldrodlength").value = document.getElementById("newrodlength").value;
		document.getElementById("oldradius").value = document.getElementById("newradius").value;
		document.getElementById("oldhomedheight").value = document.getElementById("newhomedheight").value;
		document.getElementById("oldxstop").value = document.getElementById("newxstop").value;
		document.getElementById("oldystop").value = document.getElementById("newystop").value;
		document.getElementById("oldzstop").value = document.getElementById("newzstop").value;
		document.getElementById("oldxpos").value = document.getElementById("newxpos").value;
		document.getElementById("oldypos").value = document.getElementById("newypos").value;
		document.getElementById("oldzpos").value = document.getElementById("newzpos").value;
}

// Initialisation
document.getElementById("numPoints").value = initialPoints;
document.getElementById("numfactors").value = initialFactors;
